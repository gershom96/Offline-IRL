...
class RewardModelSCAND4(nn.Module):
    def __init__(self, num_queries=4):  # Multi-query support
        ...
        self.cross_attention = nn.MultiheadAttention(
            embed_dim=self.hidden_dim, num_heads=8, batch_first=True, dropout=0.1
        )
        ...
        self.reward_head = nn.Sequential(
            ...
            nn.Linear(128, 1)#, ScaledTanh(alpha=2.0)  # Normalize output range
        )
    ...
    def forward(self, image, goal_distance, heading_error, velocity, omega, last_action, preference_ranking, batch_size):
        ...
        preference_ranking: (batch_size, 2)  # 25 ranked action pairs
        """
        ...
        state_inputs = torch.cat([goal_distance, heading_error, velocity, omega, last_action, preference_ranking], dim=-1)  # (batch_size, 8)
        ...
        state_queries = torch.stack(query_list, dim=1)  # Shape: (batch_size, Q, hidden_dim)
        state_queries = self.state_queries_norm(state_queries)

        # Cross-Attention (Querying vision features with action-specific queries)
        fused_features, _ = self.cross_attention(state_queries, attn_output, attn_output)  # Shape: (batch_size, Q, hidden_dim)
        fused_features = fused_features + state_queries
        # print(fused_features.shape)
        fused_features = fused_features.reshape(batch_size, -1) # Shape: (batch_size, Q * hidden_dim)
